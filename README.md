...
The intentionally optional `media/` directory is mounted only when present. New manager uploads are stored below `var/uploads/`; they never modify `/media`. Edition images are published only through `/assets/{edition_slug}/{public_filename}`; the storage path is never exposed.
...
- Uploads accept validated JPEG, PNG, and WebP images up to `MAX_UPLOAD_BYTES`. Pillow checks actual content, filenames are random, persistence is atomic, and APIs return versioned, edition-scoped asset URLs.
...
`python scripts/generate_assets.py` is the explicit build/one-time importer for the artwork currently shipped in the repository. It copies those files into authoritative storage, creates role-based database records, and links seeded capsule cover entries by foreign key. Application startup and the ordinary seed command never scan repository artwork or recover missing assets.
...
alembic upgrade head
python -m backend.cli seed
python scripts/check_legacy_asset_paths.py
git diff --check
git status --short
